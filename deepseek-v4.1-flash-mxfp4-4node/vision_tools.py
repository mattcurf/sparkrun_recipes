...
M = "deepseek-v4.1-flash-mxfp4-tp4"
...
def png(w, h, pixel):
    raw = b"".join(
        b"\x00" + b"".join(bytes(pixel(x, y)) for x in range(w)) for y in range(h)
    )
...
def ask(tag, parts, expect):
    ...
    answer = a.lower()
    positions = [answer.find(e) for e in expect]
    ok = all(pos >= 0 for pos in positions) and positions == sorted(positions)
    print(
        f"{tag}: {'PASS' if ok else 'CHECK'} | answer={a!r} | prompt_tokens={r['usage']['prompt_tokens']} ({dt:.1f}s)",
        flush=True,
    )
    return ok
...
ok.append(
    ask(
        ...
        [
            {
                "type": "text",
                "text": "This image has three vertical stripes. Name their colors from left to right, comma separated, lowercase.",
            },
            stripes,
        ],
        ["red", "green", "blue"],
...
ok.append(
    ask(
        ...
        [
            {
                "type": "text",
                "text": "Here are two images. What color is the first image and what color is the second? Answer as: first=<color>, second=<color>.",
            },
            red_sq,
            blue_sq,
        ],
...
ok.append(
    ask(
        ...
        [
            {
                "type": "text",
                "text": "The image is a 2x2 grid of colored squares. What color is the top-right square? One word.",
            },
            grid,
        ],
        ["green"],
...
msgs = [
    {"role": "user", "content": "What's the weather in Paris right now, in celsius?"}
]
r, dt = post(
    {
        "model": M,
...
t1 = (
    ...
    and c
    and c[0][0] == "get_weather"
    and c[0][1].get("city", "").lower() == "paris"
)
ok.append(bool(t1))
print(
    f"T1 tool call: {'PASS' if t1 else 'CHECK'} | finish={r['choices'][0]['finish_reason']} calls={[(n, a) for n, a, _ in c]} ({dt:.1f}s)",
    flush=True,
)
...
if c:
    msgs += [
        r["choices"][0]["message"],
        {
            "role": "tool",
            "tool_call_id": c[0][2],
            "content": json.dumps({"city": "Paris", "temp_c": 18, "sky": "light rain"}),
        },
    ]
    r, dt = post(
        {
    ...
    ok.append(t2)
    print(
        f"T2 round trip: {'PASS' if t2 else 'CHECK'} | answer={a!r} ({dt:.1f}s)",
        flush=True,
    )
else:
    ok.append(False)
    print("T2 round trip: CHECK | skipped because T1 returned no tool call", flush=True)
...
r, dt = post(
    {
        ...
        "messages": [
            {
                "role": "user",
                "content": "Get me the weather in Tokyo and the weather in Berlin, both in celsius.",
            }
        ],
    }
)
...
ok.append(t3)
print(
    f"T3 parallel calls: {'PASS' if t3 else 'CHECK'} | calls={[(n, a) for n, a, _ in c]} ({dt:.1f}s)",
    flush=True,
)
...
ok.append(t4)
print(
    f"T4 forced tool_choice: {'PASS' if t4 else 'CHECK'} | calls={[(n, a) for n, a, _ in c]} ({dt:.1f}s)",
    flush=True,
)
